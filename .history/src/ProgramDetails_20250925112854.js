...
const ProgramDetails = ({ userId }) => {
  ...
  // 🔹 Build dateRange properly (always as array of 'YYYY-MM-DD' strings)
  const rawDateRange = normalizeDates(program);
  const dateRange = rawDateRange.map(date => {
    if (date?.seconds) return new Date(date.seconds * 1000).toLocaleDateString("en-CA", { timeZone: "Asia/Manila" });
    if (date instanceof Date) return date.toLocaleDateString("en-CA", { timeZone: "Asia/Manila" });
                ...
                <div className="attendance-tab">
                  {userRole === "admin" || userRole === "trainer" ? (
                    <div>
                      ...
                      <button
                        onClick={() =>
                          handleDownloadAttendance(dateRange, program)
                        }
                          ...
                          <thead className="bg-gray-100">
                            <tr>
                              ...
                              {dateRange.map((dateStr) => (
                                <th key={dateStr} className="px-2 sm:px-4 py-2">
                                  {dateStr}
                                </th>
                              ))}
                            </tr>
                            ...
                            {allUsersAttendance.map(({ user_id, attendance, full_name }) => (
                              <tr key={user_id} className="border-b">
                                ...
                                {dateRange.map((dateStr) => {
                                  ...
                                  if (status === "No Data" && dateStr < today) {
                                    status = "absent";
                                  }
                                  return (
                                    <td
  ...
  return (
    <div className="program-details-fullscreen">
      ...
      {/* Ensure all parent divs are closed */}
    </div>
  );
        {IsDeleteModalOpen && (
