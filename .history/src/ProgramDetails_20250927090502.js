...
const ProgramDetails = ({ userId }) => {
  ...
  const [materials, setMaterials] = useState({ videoUrls: [], imageUrls: [], documentUrls: [] });
      ...
      try {
        ...
        if (!typeSnapshot.empty) {
          ...
          setMaterials({
            ...
            documentUrls: data.documentUrls || [] // Add document URLs
          });
        } else {
          setMaterials({ videoUrls: [], imageUrls: [], documentUrls: [] }); // No materials found
        }
      } catch (error) {
        console.error("Error fetching materials:", error);
        // Show error notification to user
        Swal.fire('Error', 'Failed to load training materials. Please try again later.', 'error');
      }
      setLoading(false);
    };
    ...
    <div className="program-details-fullscreen">
      <div className="program-details-content">
        ...
        <div className="flex flex-wrap gap-3 mt-6">
          {userRole === "user" && (
            <>
              {/* For approved users - Show mark attendance button */}
                            ...
                            {approvedApplicants
                              ...
                              .map((applicant) => (
                                <tr key={`${applicant.user_id}_${applicant.program_id}`} className="border-b">
                                  ...
                                  <td className="px-2 sm:px-4 py-2 flex flex-col sm:flex-row justify-center gap-2">
                                    <button
        ...
        {showQRModal && (
          ...
          >
            <div 
              className="bg-white rounded-2xl shadow-2xl max-w-sm w-full mx-auto transform transition-all duration-300 scale-100 opacity-100"
                ...
                {formattedRelevantDate ? (
                  <div className="space-y-6">
                    ...
                    <div className="bg-blue-50 rounded-xl p-4">
                      <p className="text-sm text-blue-600 font-medium mb-1">Training Date</p>
                    ...
                    {/* QR Code */}
                    <div className="flex justify-center p-4 bg-white rounded-xl border-2 border-dashed border-gray-200">
                    ...
                    <div className="text-center text-sm text-gray-500">
                      Scan this QR code using your device's camera to mark attendance
                    </div>
                  </div>
                ) : (
                  <div className="flex flex-col items-center justify-center py-8 px-4">
                    ...
                    <p className="text-gray-500 text-sm mt-2 text-center">Please check the training schedule</p>
                  </div>
                )}
              </div>
          ...
          {(materials.videoUrls.length > 0 || materials.imageUrls.length > 0) && (
            <div className="mb-6">
              ...
              <div className="relative">
                ...
                {(materials.videoUrls.length + materials.imageUrls.length) > 1 && (
                  <>
                    <button
                    ...
                    <div className="flex justify-center gap-2 mt-4">
                      {Array.from({ length: materials.videoUrls.length + materials.imageUrls.length }).map((_, index) => (
                        <button
                          key={index}
                  ...
                  try {
                    ...
                    return (
                      <a
                        ...
                        className="flex items-center p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-all transform hover:-translate-y-1 hover:shadow-md group"
                        onClick={(e) => {
                          if (!doc.startsWith('http')) {
                            e.preventDefault();
                            Swal.fire('Error', 'Invalid document link. Please contact support.', 'error');
                          }
                        }}
                      >
                        ...
                        <div className="ml-3 flex-1 min-w-0">
                          <span className="text-sm text-gray-700 font-medium group-hover:text-gray-900 transition-colors truncate block">
                            {fileName}
                          </span>
