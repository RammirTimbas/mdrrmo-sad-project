...
const ProgramDetails = ({ userId }) => {
  ...
  const handleApply = async () => {
    ...
    let programSnapshot;
    try {
      programSnapshot = await getDoc(programRef);
    } catch (err) {
      Swal.fire("Error", "Could not fetch program info.", "error");
      return;
    }
    const currentSlots = programSnapshot.data()?.slots ?? 0;
    if (currentSlots <= 0) {
      Swal.fire({
        title: "No Slots Available",
        text: "Sorry, this program is already full.",
        icon: "warning",
        confirmButtonText: "OK",
      });
      return;
    ...
    try {
      // ...existing code for userInfo, programData, etc...
      // ✅ SAVE APPLICATION
      const applicantData = {
        user_id: userId,
