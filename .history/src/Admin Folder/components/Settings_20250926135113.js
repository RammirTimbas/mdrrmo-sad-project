...
const Settings = ({ userId }) => {
  ...
  const AccordionItem = ({ id, title, children, icon, openSection, setOpenSection }) => (
    <div className="border border-gray-300 rounded-lg shadow-md bg-white overflow-hidden">
      ...
      <div
        ...
        style={{
          maxHeight: openSection === id ? 'none' : 0,
  ...
  const handleFileUpload = async (e, type, fileType) => {
    ...
    try {
      ...
      // Create storage reference with correct path for each file type
      const storageRef = ref(
        storage,
        `training-materials/${type}/${fileType === 'document' ? 'documents' : fileType + 's'}/${file.name}`
      );
    ...
    try {
      ...
      if (!typeSnapshot.empty) {
        ...
        setMaterials({
          ...
          documentUrls: typeDoc.data().documentUrls || []
        });
      } else {
        setMaterials({ imageUrls: [], videoUrls: [], documentUrls: [] });
      }
    } catch (error) {
      ...
      {activeTab === "carouselImages" && (
        <div className="bg-white rounded-xl shadow-lg p-6 min-h-[600px]">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-2xl font-semibold text-gray-800 mb-2">Manage Carousel Images</h2>
          ...
          <div className="carousel-content">
            {carouselImages.length > 0 ? (
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <div className="absolute bottom-0 left-0 right-0 p-4">
                        ...
                        <button
                          ...
                          className="w-full px-3 py-2 bg-red-500 hover:bg-red-600 text-white text-sm rounded-lg transition-colors duration-200 flex items-center justify-center gap-2"
                        >
              ...
              <div className="flex flex-col items-center justify-center min-h-[400px] text-gray-500">
                ...
                <p className="text-sm">Click the Add Image button to upload your first carousel image</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="p-6 bg-white shadow-lg rounded-lg">
          ...
          <div className="space-y-4">
            <AccordionItem
              ...
              icon={
                // Pie chart icon for quota
                <svg className="w-6 h-6 text-blue-500" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M11 3.055A9 9 0 1021 12h-9V3.055z" /><path strokeLinecap="round" strokeLinejoin="round" d="M20.488 9H13V2.512A9.025 9.025 0 0120.488 9z" /></svg>
              }
            ...
            <AccordionItem
              ...
              icon={
                // Users/group icon for population
                <svg className="w-6 h-6 text-green-600" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m9-4a4 4 0 11-8 0 4 4 0 018 0zm6 6v2a2 2 0 01-2 2h-1.5M3 16v2a2 2 0 002 2h1.5" /></svg>
              }
              openSection={openSection}
              setOpenSection={setOpenSection}
            >
            ...
            <AccordionItem
              ...
              icon={
                // Book icon for training materials
                <svg className="w-6 h-6 text-yellow-600" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 20h9" /><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m0 0H3a1 1 0 01-1-1V5a1 1 0 011-1h9z" /></svg>
              }
              openSection={openSection}
              setOpenSection={setOpenSection}
            >
                  ...
                  <div className="mt-6">
                    ...
                    <div className="mb-8 bg-gray-50 p-4 rounded-lg">
                      ...
                      <input
                        ...
                        className="w-full p-2 border rounded-lg focus:ring focus:ring-blue-400 mb-3"
                      />
                      {uploadProgress.image !== null && (
                        <div className="mb-4 w-full bg-gray-200 rounded-full h-2 dark:bg-gray-700">
                          <div className="bg-blue-600 h-2 rounded-full transition-all" style={{ width: `${uploadProgress.image}%` }}></div>
                          <span className="text-xs text-gray-500 mt-1">{uploadProgress.image}% uploaded</span>
                        </div>
                      )}
                    ...
                    <div className="mb-8 bg-gray-50 p-4 rounded-lg">
                      ...
                      {uploadProgress.video !== null && (
                        <div className="mb-4 w-full bg-gray-200 rounded-full h-2 dark:bg-gray-700">
                          <div className="bg-green-600 h-2 rounded-full transition-all" style={{ width: `${uploadProgress.video}%` }}></div>
                          <span className="text-xs text-gray-500 mt-1">{uploadProgress.video}% uploaded</span>
                        </div>
                      )}
                    ...
                    <div className="mb-8 bg-gray-50 p-4 rounded-lg">
                      ...
                      <input
                        ...
                        onChange={(e) => handleFileUpload(e, selectedType, "document")}
                        className="w-full p-2 border rounded-lg focus:ring focus:ring-blue-400 mb-3"
                      />
                      {uploadProgress.document !== null && (
                        <div className="mb-4 w-full bg-gray-200 rounded-full h-2 dark:bg-gray-700">
                          <div className="bg-yellow-600 h-2 rounded-full transition-all" style={{ width: `${uploadProgress.document}%` }}></div>
                          <span className="text-xs text-gray-500 mt-1">{uploadProgress.document}% uploaded</span>
                        </div>
                      )}
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {materials.documentUrls?.map((docUrl, index) => (
                          <div key={index} className="relative group bg-white p-4 rounded-lg border border-gray-200 hover:shadow-lg transition-shadow">
                            ...
                            <div className="absolute top-0 right-0 p-2 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button
                                onClick={() => handleDeleteFile("document", docUrl)}
