...
const PopulationConfig = () => {
  ...
  const handleMunicipalityChange = (municipality, value) => {
    ...
    const totalPop = parseInt(value, 10);
    const barangayTotal = Object.values(
      populationData[municipality]?.barangays || {}
    ).reduce((sum, pop) => sum + (parseInt(pop, 10) || 0), 0);

    // Allow editing (deleting digits) if the new value is a prefix of the old value
    const prevValue = String(populationData[municipality]?.totalPopulation ?? populationData[municipality]?.total_population ?? "");
    if (totalPop < barangayTotal && !prevValue.startsWith(value)) {
      Swal.fire(
        "Error",
    ...
    setPopulationData((prevData) => ({
      ...
      [municipality]: {
        ...prevData[municipality],
        totalPopulation: value === "" ? "" : totalPop, // allow string for editing
        total_population: value === "" ? "" : totalPop,
      },
    }));
  };
