...
const CompletedApplicantsMap = () => {
  ...
  const [selectedMunicipalityCoords, setSelectedMunicipalityCoords] = useState(null);
  ...
  const [filteredDemographics, setFilteredDemographics] = useState(defaultDemographics);

  const CACHE_KEY = "applicantsCache";
  const CACHE_EXPIRY_KEY = "applicantsCacheExpiry";
  const CACHE_DURATION = 30 * 60 * 1000; // 30 minutes
  // Fetch coverageConfig from Firestore
  useEffect(() => {
    const fetchCoverageConfig = async () => {
      try {
        ...
        if (populationSnap.exists()) {
          ...
          if (data.coverageConfig) {
            setCoverageConfig(data.coverageConfig);
          }
        }
      } catch (error) {
        console.error("Error fetching coverageConfig:", error);
      }
    };
    fetchCoverageConfig();
  }, []);
  ...
  useEffect(() => {
    const fetchCompletedApplicants = async () => {
      ...
      if (cachedData && cacheExpiry && Date.now() < parseInt(cacheExpiry)) {
        ...
        setMunicipalityStats(parsedData.stats);
  ...
  const geocodeAddress = async (
    ...
    const addressVariants = [
      `${purok ? `Purok ${purok}, ` : ""}${street ? street + ", " : ""}${
        barangay ? barangay + ", " : ""
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Object.keys(municipalityStats).map((municipality) => {
                ...
                if (hasMunicipalityPopulation) {
                  const sortedThresholds = [...(coverageConfig.municipality || [])].sort((a, b) => b.threshold - a.threshold);
                  for (const item of sortedThresholds) {
                    if (municipalityCoverage >= item.threshold) {
                      municipalityInsight = item.message;
                      break;
                    }
                  }
                }
                      ...
                      <div className="mt-3 bg-white rounded-md p-3">
                        ...
                        {Object.keys(municipalityData.barangays).map(
                          (barangay) => {
                            ...
                            // Use dynamic coverageConfig for barangay
                            let barangayInsight = "⚠ Population Data Not Set";
                            if (hasBarangayPopulation) {
                              const sortedThresholds = [...(coverageConfig.barangay || [])].sort((a, b) => b.threshold - a.threshold);
                              for (const item of sortedThresholds) {
                                if (barangayCoverage >= item.threshold) {
                                  barangayInsight = item.message;
                                  break;
                                }
                              }
                            }
