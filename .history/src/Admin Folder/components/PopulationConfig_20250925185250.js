...
const PopulationConfig = () => {
  ...
  const handleMunicipalityChange = (municipality, value) => {
    // Allow empty string for input (user is editing)
    if (value === "") {
    ...
    // Only validate if input is not empty and user is not in the middle of editing (allow if value is being cleared)
    // If user is setting a value (not clearing), enforce the restriction
    if (value !== "" && totalPop < barangayTotal) {
      // If user is deleting digits (e.g., backspace), allow if the new value is a prefix of the old value
      const prevValue = populationData[municipality]?.totalPopulation ?? populationData[municipality]?.total_population ?? "";
      if (String(prevValue).startsWith(value)) {
        // Allow editing (user is deleting digits)
        setPopulationData((prevData) => ({
          ...prevData,
          [municipality]: {
            ...prevData[municipality],
    ...
    setPopulationData((prevData) => ({
      ...
      [municipality]: {
        ...prevData[municipality],
        totalPopulation: totalPop, // camelCase for compatibility
        total_population: totalPop, // snake_case for compatibility
      },
    }));
  };
  ...
  const handleBarangayChange = (municipality, barangay, value) => {
    ...
    // Accept both camelCase and snake_case for total population
    const totalPop = populationData[municipality]?.totalPopulation ?? populationData[municipality]?.total_population ?? 0;
          ...
          {Object.keys(populationData).map((municipality) => (
            ...
            >
              ...
              <input
                type="number"
                min="1"
                value={populationData[municipality]?.totalPopulation ?? populationData[municipality]?.total_population ?? ""}
                onChange={(e) =>
