...
const Settings = ({ userId }) => {
  ...
  const handleFileUpload = async (e, type, fileType) => {
    ...
    try {
      ...
      const storageRef = ref(storage, `training_materials/${selectedType}/${fileType}/${file.name}`);
      ...
      uploadTask.on('state_changed',
        ...
        async () => {
          ...
          // Update the materials state based on file type
          setMaterials(prevMaterials => {
            const newMaterials = { ...prevMaterials };

            if (fileType === 'image') {
              newMaterials.imageUrls = [...(prevMaterials.imageUrls || []), downloadURL];
            } else if (fileType === 'video') {
              newMaterials.videoUrls = [...(prevMaterials.videoUrls || []), downloadURL];
            } else if (fileType === 'document') {
              newMaterials.documentUrls = [...(prevMaterials.documentUrls || []), downloadURL];
            }
          ...
          // Save to Firestore
          const materialsRef = doc(db, 'training_materials', selectedType);
          await setDoc(materialsRef, {
            type: selectedType,
            [`${fileType}Urls`]: arrayUnion(downloadURL)
          }, { merge: true });
