...
const CompletedApplicantsMap = () => {
  ...
  // Fetch coverageConfig and all applicant/population data (on mount and on demand)
  const fetchAllData = async () => {
    Swal.fire({
      title: "Reloading Settings...",
      text: "Please wait while the latest settings are loaded.",
    ...
    try {
      ...
      // Fetch coverageConfig
      const populationRef = doc(db, "Settings", "Populations");
      const populationSnap = await getDoc(populationRef);
      if (populationSnap.exists()) {
        const data = populationSnap.data();
        if (data.coverageConfig) setCoverageConfig(data.coverageConfig);
      }

      // Fetch applicants and stats (reuse logic from fetchCompletedApplicants)
      // (Copy the body of fetchCompletedApplicants here, but setLoading is already true)
      ...
      programs.forEach((program) => {
        if (program.approved_applicants) {
          program.approved_applicants.forEach((applicant) => {
            if (!applicantMap[applicant.user_id]) {
              ...
              applicantMap[applicant.user_id].programs.push(program.title);
            }
          });
        }
      });
      ...
      // Fetch population data again (for stats)
      const populationSnap2 = await getDoc(populationRef);
      const populationData = populationSnap2.exists()
        ? populationSnap2.data().municipalities || {}
        : {};
      ...
      allApplicants.forEach((applicant) => {
        ...
        const municipality = userInfo.municipality;
        const barangay = userInfo.barangay;
        if (!municipality) return;
        if (!stats[municipality]) {
          stats[municipality] = {
            totalApplicants: 0,
            totalPopulation: populationData[municipality]?.totalPopulation || 0,
            barangays: {},
          };
        }
        ...
        if (barangay) {
          if (!stats[municipality].barangays[barangay]) {
            stats[municipality].barangays[barangay] = {
              applicants: 0,
              totalPopulation:
                populationData[municipality]?.barangays?.[barangay] || 0,
            };
          }
      ...
      const geocodedApplicants = await Promise.all(
        applicantsWithLocation.map(async (applicant) => {
          if (applicant.lat && applicant.lng) return applicant;
          ...
          if (geo) {
            return { ...applicant, lat: geo.lat, lng: geo.lng };
          }
          return applicant;
        })
      );
      ...
      setLoading(false);
      Swal.close();
      console.error("Error reloading all data:", e);
      Swal.fire("Error", "Failed to reload settings.", "error");
    }
  };
  useEffect(() => {
    fetchAllData();
