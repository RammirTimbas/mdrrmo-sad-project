// --- CoverageConfig dynamic thresholds/messages ---
...
// Fetch coverageConfig from Firestore on mount
useEffect(() => {
  const fetchCoverageConfig = async () => {
    try {
      ...
      console.error("Error fetching coverageConfig:", e);
    }
  };
  fetchCoverageConfig();
}, []);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Object.keys(municipalityStats).map((municipality) => {
                ...
                if (hasMunicipalityPopulation) {
                  if (municipalityCoverage > 10) {
                    municipalityInsight =
                      "🟢 Good Coverage - Well-represented in training.";
                  } else if (municipalityCoverage >= 5) {
                    municipalityInsight =
                      "🟡 Moderate Coverage - Consider additional programs.";
                  } else {
                    municipalityInsight =
                      "🔴 Low Coverage - Needs urgent intervention.";
                  }
                }
                      ...
                      <div className="mt-3 bg-white rounded-md p-3">
                        ...
                        {Object.keys(municipalityData.barangays).map(
                          (barangay) => {
                            ...
                            // Use dynamic coverageConfig.barangay
                            let barangayInsight = "⚠ Population Data Not Set";
                            if (hasBarangayPopulation) {
                              const sorted = [...(coverageConfig.barangay || [])].sort((a, b) => b.threshold - a.threshold);
