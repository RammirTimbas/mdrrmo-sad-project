...
const TrainingPrograms = ({ userId }) => {
  ...
  const [trainerNames, setTrainerNames] = useState([]); // all trainers
  const [availableTrainers, setAvailableTrainers] = useState([]); // filtered by date
  ...
  // Fetch training types and all trainers (once)
  useEffect(() => {
    ...
    fetchAllTrainers();
    return () => unsubscribeTrainingTypes();
  }, []);
      ...
      try {
        ...
        const isTrainerOccupied = (trainer, selectedDates, startDate, endDate, dateMode) => {
          for (const program of allPrograms) {
            // Handle array or string for trainer_assigned
                ...
                <div className="form-group">
                  ...
                  >
                    <option value="">{(newProgram.dateMode === "range" && (!newProgram.start_date || !newProgram.end_date)) ||
                      (newProgram.dateMode === "specific" && (!newProgram.selected_dates || newProgram.selected_dates.length === 0))
                      ? "Select date(s) first" : (availableTrainers.length === 0 ? "No trainers available" : "Select Trainer")}</option>
