...
const PopulationConfig = () => {
  ...
  const handleMunicipalityChange = (municipality, value) => {
    ...
    setPopulationData((prevData) => ({
      ...
      [municipality]: {
        ...
        totalPopulation: value === "" ? "" : parseInt(value) || 0,
        total_population: value === "" ? "" : parseInt(value) || 0,
      },
    }));
  };
  ...
  const savePopulationData = async () => {
    // Restriction: total population must be ≥ sum of barangays for each municipality
    for (const municipality of Object.keys(populationData)) {
      const muniData = populationData[municipality];
      const totalPop =
        muniData.totalPopulation !== undefined && muniData.totalPopulation !== ""
          ? parseInt(muniData.totalPopulation)
          : muniData.total_population !== undefined && muniData.total_population !== ""
          ? parseInt(muniData.total_population)
          : 0;
      const barangayTotal = Object.values(muniData.barangays || {}).reduce(
        (sum, pop) => sum + (parseInt(pop) || 0),
        0
      );
      if (totalPop < barangayTotal) {
        Swal.fire(
          "Error",
          `Total population for '${municipality}' must be ≥ sum of its barangays! (Total: ${totalPop}, Barangays: ${barangayTotal})`,
          "error"
        );
        return;
      }
