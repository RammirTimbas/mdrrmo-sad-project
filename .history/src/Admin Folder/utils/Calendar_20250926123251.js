...
const Calendar = ({ programs, mobile = false, disableEventClick = false }) => {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto" style={{ background: 'linear-gradient(135deg, #e0e7ff 0%, #f0fdfa 100%)' }}>
      ...
      {filtersVisible && (
        <div className={`${mobile ? 'space-y-4' : 'grid grid-cols-1 sm:grid-cols-3 gap-4'} mb-6 px-4 sm:px-0 animate-fade-in`}>
          <FilterDropdown
            label="Trainer"
      ...
      <div className={`w-full rounded-3xl shadow-2xl bg-white/70 backdrop-blur-md ${mobile ? 'p-2' : 'p-3 sm:p-6'} transition-all duration-500`}>
        <FullCalendar
...
const FilterDropdown = ({ label, options, value, onChange, mobile }) => (
  <div className={mobile ? "bg-white/80 backdrop-blur p-3 rounded-lg shadow-md" : ""}>
    <label className="block text-sm font-medium mb-2 text-gray-700">{label}</label>
    <select
      value={value}
      onChange={(e) => onChange(e.target.value)}
      className={`w-full border border-gray-300 p-2 rounded-lg ${mobile ? 'text-base h-12' : ''}`}
    >
