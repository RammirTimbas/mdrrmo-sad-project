...
// Only one Calendar component should exist. Remove this duplicate if another exists below.
  ...
  return (
    <div className="p-0 sm:p-6 max-w-7xl mx-auto relative min-h-screen" style={{ background: 'linear-gradient(135deg, #e0e7ff 0%, #f0fdfa 100%)', minHeight: '100dvh' }}>
  ...
  {!mobile && (
      ...
      <div className="rounded-3xl shadow-2xl bg-white/70 backdrop-blur-md p-2 sm:p-6 transition-all duration-500">
        <FullCalendar
...
// Only one FilterDropdown component should exist. Remove this duplicate if another exists below.
  <div>
...
function renderEventContent(eventInfo) {
  ...
  minWidth: '120px',
    ...
    >
  <h4 style={{ fontWeight: 700, fontSize: '1.1em', letterSpacing: '0.01em', textShadow: '0 1px 4px #0002' }}>{title}</h4>
...
// Only one export default Calendar; should exist. Remove this duplicate if another exists below.
import React, { useState, useEffect, useMemo } from "react";
import { useNavigate } from "react-router-dom";
import FullCalendar from "@fullcalendar/react";
import timelinePlugin from "@fullcalendar/timeline";
  ...
  useEffect(() => {
    ...
    programs.forEach((program) => {
      const trainerList = Array.isArray(program.trainer_assigned)
        ? program.trainer_assigned
        : [program.trainer_assigned];
// Removed all leftover/duplicate JSX after the main Calendar component. Only one export and no stray JSX remain.
            label="Venue"
            options={venueOptions}
            value={selectedVenue}
            onChange={setSelectedVenue}
