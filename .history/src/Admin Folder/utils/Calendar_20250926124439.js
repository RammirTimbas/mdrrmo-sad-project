...
const Calendar = ({ programs, mobile = false, disableEventClick = false }) => {
  ...
  const handleViewClick = (program) => {
    ...
    // If accessed from user panel, go to user route, otherwise go to admin route
    const routePrefix = path.includes('/user') ? '/user/home' : '/admin/training-programs';
    navigate(`${routePrefix}/${program.id}`, { state: { program } });
  };

  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto max-h-[90vh] overflow-y-auto" style={{ 
      background: 'linear-gradient(135deg, #e0e7ff 0%, #f0fdfa 100%)',
      scrollbarWidth: 'thin',
      scrollbarColor: '#CBD5E1 transparent'
    }}>
      <div className="w-full flex justify-between items-center mb-6 sm:mb-8 sticky top-0 z-10 bg-gradient-to-r from-blue-50 to-cyan-50 p-4 rounded-lg backdrop-blur-sm">
        <h2 className="text-2xl sm:text-3xl font-bold text-blue-800 tracking-tight drop-shadow-lg">Training Schedule</h2>
        <button
      ...
      <div className="w-full rounded-3xl shadow-2xl bg-white/70 backdrop-blur-md p-3 sm:p-6 transition-all duration-500 overflow-hidden">
        <FullCalendar
