        ...
        onSnapshot(programRef, async (programDoc) => {
          try {
            ...
            const tableDates = (programData.selected_dates?.length > 0 ? programData.selected_dates : normalizeDates(programData)).map(date => {
              ...
              if (date._seconds) return new Date(date._seconds * 1000).toLocaleDateString("en-CA");
              if (typeof date === "number") return new Date(date * 1000).toLocaleDateString("en-CA");
            ...
            const allAttendance = applicantsList.filter(a => a !== null).map(applicant => {
              // Map attendance records to date string
              const attendanceMap = {};
              if (Array.isArray(applicant.attendance)) {
                applicant.attendance.forEach(record => {
                  if (record.date) attendanceMap[record.date] = record.remark;
                });
              }
              return {
                user_id: applicant.user_id,
