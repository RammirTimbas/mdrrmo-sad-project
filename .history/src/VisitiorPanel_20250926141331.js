...
const VisitorPanel = ({ onLoginClick }) => {
  ...
  const getProgramDateEarliest = (program) => {
    ...
    if (program.start_date && program.end_date) {
      ...
      return formatShortDate(endMs); // Show end date if ended
    }
    ...
    if (program.selected_dates?.length > 0) {
      ...
      const futureDates = program.selected_dates
        ...
        .filter(ms => {
          ...
          const dateEnd = new Date(ms).setHours(23, 59, 59, 999);
          return dateEnd >= now; // Include if the day hasn't fully ended
        });
      
      if (futureDates.length > 0) {
        const nextDate = Math.min(...futureDates);
        const days = Math.ceil((nextDate - now) / (1000 * 60 * 60 * 24));
        const isToday = new Date(nextDate).toDateString() === new Date(now).toDateString();
    ...
    bg-[length:200%_200%] animate-gradient"
          ...
          <div className="mt-8 flex gap-4">
            ...
            <button
              ...
              className="bg-gradient-to-r from-blue-500 via-blue-600 to-indigo-600 text-white px-6 py-3 rounded-full font-semibold 
