...
export default function ChatUI({ userId }) {
  ...
  return (
    ...
    >
      <div className="flex-1 overflow-y-auto mb-2 sm:mb-4 space-y-3 px-2 sm:px-4 py-2" style={{ fontSize: "clamp(1.5rem, 8vw, 2.1rem)" }}>
          ...
          >
            <div
              className={`transition-all duration-500 ease-in-out transform ${
                msg.sender === "user"
                  ? "bg-blue-900 text-white animate-slide-in-right"
                  : "bg-gray-800 text-white animate-slide-in-left"
              } p-4 sm:p-5 rounded-2xl max-w-[95vw] sm:max-w-lg break-words shadow-lg`}
              style={{ fontSize: "clamp(1.6rem, 9vw, 2.2rem)", wordBreak: 'break-word', color: '#fff' }}
            >
              {msg.text.split("\n").map((line, i) => {
                ...
                return (
                  <p
                    key={i}
                    className={`whitespace-pre-wrap ${isTitle ? "font-bold text-yellow-300" : ""}`}
                    style={{ fontSize: isTitle ? "clamp(1.7rem, 10vw, 2.3rem)" : undefined, color: '#fff' }}
                  >
                    {line}
                  </p>
                );
        ...
        {isBotTyping && (
          <div className="flex justify-start">
            <div className="bg-gray-800 text-white p-4 sm:p-5 rounded-2xl max-w-[95vw] sm:max-w-lg break-words shadow-lg flex items-center gap-2 animate-fade-in">
              <span>Typing</span>
