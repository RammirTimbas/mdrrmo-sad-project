...
const History = ({ userId }) => {
  ...
  const handleRateClick = async (event, programId) => {
    ...
    try {
      ...
      const attendanceRef = collection(programRef, "attendance");
      const attendanceQuery = query(attendanceRef, where("userId", "==", userId));
      const attendanceSnapshot = await getDocs(attendanceQuery);
      
      const totalDates = programData.selected_dates?.length || 0;
      let attendedDates = 0;
      ...
      const attendancePercentage = (attendedDates / totalDates) * 100;
      
      if (attendancePercentage < 80) {
        Swal.fire({
          title: "Insufficient Attendance",
          text: "You need at least 80% attendance to request a certificate. Your current attendance is " + attendancePercentage.toFixed(1) + "%",
          icon: "warning",
