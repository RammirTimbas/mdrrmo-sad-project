...
const History = ({ userId }) => {
  ...
  const handleRateClick = async (event, programId) => {
    ...
    try {
      // Get program data including attendance
      const programRef = doc(db, "Training Programs", programId.id);
      const programDoc = await getDoc(programRef);
      const programData = programDoc.data();
      
      // Get attendance from program data
      const attendanceMap = programData.attendance || {};
      const userAttendance = Object.values(attendanceMap).filter(record => 
      ...
      // Get the program details to check dates
      const programRef = doc(db, "Training Programs", programId.id);
      const programDoc = await getDoc(programRef);
      const programData = programDoc.data();

      // Check if program uses selected_dates or start/end date
      ...
      attendanceSnapshot.forEach(doc => {
        const data = doc.data();
        if (data.status === "present" || data.isPresent === true) { // Check both status formats
          attendedDates++;
        }
      });
      ...
      if (attendedDates < totalDates) {
        Swal.fire({
          ...
          text: `Full attendance is required to request a certificate. Your attendance: ${attendedDates}/${totalDates} days`,
          icon: "warning",
