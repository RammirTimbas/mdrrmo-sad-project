...
const RatingForm = ({ programId, userId, onClose, onSubmit }) => {
  ...
  const requestCertificate = async () => {
    try {
      ...
      console.log("🔍 Date Processing Start:");
      console.log("Selected dates:", programId.selected_dates);
      console.log("Start date:", programId.start_date);

      // Helper function to convert any date format to milliseconds
      const getMilliseconds = (date) => {
        ...
        if (date && typeof date.seconds === 'number') {
          ...
          console.log(`Converting Firestore Timestamp to milliseconds: ${milliseconds}`);
          return milliseconds;
        }
        ...
        if (date && typeof date === 'number') {
          ...
          console.log(`Converting Unix timestamp to milliseconds: ${milliseconds}`);
          return milliseconds;
        }
        
        // Handle other date formats (fallback)
        const timestamp = new Date(date).getTime();
        console.log(`Converting date to timestamp: ${timestamp}`);
        return timestamp;
      };
      ...
      if (programId.selected_dates && programId.selected_dates.length > 0) {
        try {
          ...
          const timestamps = programId.selected_dates.map(getMilliseconds);
          
          const earliestDate = Math.min(...timestamps);
          console.log("Earliest timestamp:", earliestDate);
          ...
          if (isNaN(dateObj.getTime())) {
            ...
            dateHumanReadable = "N/A";
          } else {
            dateHumanReadable = dateObj.toLocaleDateString("en-US", {
              year: "numeric",
              month: "long",
              day: "numeric",
            });
            console.log("✅ Formatted date:", dateHumanReadable);
          }
        } catch (error) {
        ...
        try {
          // Use the same helper function for start_date
          const milliseconds = getMilliseconds(programId.start_date);
          console.log("Start date milliseconds:", milliseconds);
