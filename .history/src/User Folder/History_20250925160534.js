...
const History = ({ userId }) => {
  ...
  const handleRateClick = async (event, programId) => {
    ...
    try {
      ...
      // Format program date as readable string
      let programDate = '';
      if (programData.selected_dates && programData.selected_dates.length > 0) {
        // For multiple specific dates, format and join them as "Month Day, Year"
        ...
        // For date range, format as "Month Day, Year to Month Day, Year"
        const startStr = new Date(programData.start_date * 1000).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
        const endStr = new Date(programData.end_date * 1000).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
        programDate = `${startStr} to ${endStr}`;
      } else {
        programDate = '';
      }
