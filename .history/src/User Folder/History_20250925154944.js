...
const History = ({ userId }) => {
  ...
  const handleRateClick = async (event, programId) => {
    ...
    try {
      ...
      // Prepare training dates string
      let trainingDates = '';
      if (programData.selected_dates && programData.selected_dates.length > 0) {
        // For multiple specific dates, format and join them
        trainingDates = programData.selected_dates
          .map(date => formatDate(date.seconds))
          .join(', ');
      } else if (programData.start_date && programData.end_date) {
        // For date range, format as start to end
        trainingDates = `${formatDate(programData.start_date)} to ${formatDate(programData.end_date)}`;
      }
      ...
      const certificateData = {
        ...
        participantName: userApplicant.full_name,
