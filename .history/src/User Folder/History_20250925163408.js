...
const History = ({ userId }) => {
  ...
  const handleRateClick = async (event, programId) => {
    ...
    try {
      ...
      // Create certificate data object with both raw and formatted dates
      const certificateData = {
        ...
        // Store both raw dates and formatted string
        dates: programData.selected_dates
          ? { type: 'selected', dates: programData.selected_dates }
          : { type: 'range', start: programData.start_date, end: programData.end_date },
        programDate: programData.selected_dates?.length > 0
          ? programData.selected_dates
            .map(date => new Date(date.seconds * 1000).toLocaleDateString('en-US', {
              month: 'long',
              day: 'numeric',
              year: 'numeric'
            }))
            .join(', ')
          : `${new Date(programData.start_date * 1000).toLocaleDateString('en-US', {
            month: 'long',
            day: 'numeric',
            year: 'numeric'
          })} to ${new Date(programData.end_date * 1000).toLocaleDateString('en-US', {
            month: 'long',
            day: 'numeric',
            year: 'numeric'
          })}`,
  ...
  const syncAllToGoogleCalendar = async () => {
    ...
    // Helper to check if any selected_dates are in the future or ongoing
    function hasFutureSelectedDate(program) {
      if (!program.selected_dates || program.selected_dates.length === 0) return false;
      
      console.log("Checking selected dates program:", program.program_title);
      return program.selected_dates.some(dateObj => {
        ...
        if (dateObj && typeof dateObj.seconds === 'number') {
          // Firestore Timestamp
          date = new Date(dateObj.seconds * 1000 + Math.floor(dateObj.nanoseconds / 1000000));
          console.log(`Selected date (Firestore): ${date}`);
        } else if (typeof dateObj === 'number') {
          // Unix timestamp
          date = new Date(dateObj * 1000);
          console.log(`Selected date (Unix): ${date}`);
        } else {
          // Regular date object
          date = new Date(dateObj);
          console.log(`Selected date (Regular): ${date}`);
        }
    ...
    // Helper to check if ranged program is in the future or ongoing
    function isFutureRangedProgram(program) {
      if (program.start_date && program.end_date) {
        console.log("Checking ranged program:", program.program_title);
        
        const endDate = new Date(program.end_date * 1000);
        const startDate = new Date(program.start_date * 1000);
        console.log(`Start date: ${startDate}, End date: ${endDate}`);
        
        // Program is considered future/ongoing if either:
        // 1. End date is in the future
        // 2. Current date is between start and end dates
        const isFutureOrOngoing = endDate >= now || (now >= startDate && now <= endDate);
        console.log(`Is future or ongoing: ${isFutureOrOngoing}`);
