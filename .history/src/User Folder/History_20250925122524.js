...
const History = ({ userId }) => {
  ...
  const handleRateClick = async (event, programId) => {
    ...
    try {
      ...
      const attendanceRecords = programData.attendance || {};
      
      // Count days where the user was present
      Object.values(attendanceRecords).forEach(record => {
        if (record.userId === userId && record.isPresent === true) {
          attendedDates++;
        }
      });      if (attendedDates < totalDates) {
        Swal.fire({
          title: "Incomplete Attendance",
          text: `Your attendance: ${attendedDates} out of ${totalDates} days. You must have complete attendance to request a certificate.`,
          icon: "warning",
      ...
      if (attendedDates === 0) {
        console.error("No attendance records found for user:", userId);
      }
