            ...
            {activeTab === "attendance" && (
              <div className="table-container bg-white shadow-lg rounded-lg p-4 sm:p-6">
                <div className="attendance-tab">
                  {/* Attendance Table for all roles: show all dates in range as columns, present/absent for each user/date */}
                  <div>
                          ...
                          {approvedApplicants.map(applicant => (
                            <tr key={applicant.user_id}>
                              ...
                              {dateRange.map((date, idx) => {
                                ...
                                const attendanceRecord = (allUsersAttendance.find(a => a.user_id === applicant.user_id)?.attendance || []).find(a => {
                                  let attDate = a.date;
                                  if (attDate instanceof Date) attDate = attDate.toISOString().split("T")[0];
                                  return attDate === dateStr;
                                });
                                return (
                                  <td key={idx} className={`px-2 sm:px-4 py-2 capitalize text-center ${attendanceRecord?.remark === "present" ? "text-green-600" : "text-red-600"}`}>
                                    {attendanceRecord ? attendanceRecord.remark : "absent"}
                                  </td>
                                );
                              })}
                            </tr>
