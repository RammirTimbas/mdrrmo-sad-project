...
const ProgramDetails = ({ userId }) => {
  ...
  const normalizeDates = (program) => {
    if (program.dateMode === "range") {
      if (program.start_date && program.end_date) {
        // Generate all dates between start and end (inclusive) in UTC
  ...
  useEffect(() => {
    ...
    // get approved applicants and attendance
      ...
      try {
        ...
        onSnapshot(programRef, async (programDoc) => {
          try {
            ...
            const applicantsList = await Promise.all(
              Object.entries(approved).map(
                async ([applicationId, applicant]) => {
                  if (!applicant.user_id) return null;
                  try {
            ...
            setApprovedApplicants(applicantsList.filter((applicant) => applicant !== null));

            // Build attendance data for table
            let tableDates = [];
            if (programData.selected_dates?.length > 0) {
              tableDates = programData.selected_dates.map(date => {
              ...
              while (current <= endDate) {
                tableDates.push(new Date(current));
                current.setDate(current.getDate() + 1);
              }
            }

            const allAttendance = applicantsList.filter(a => a !== null).map(applicant => {
              // Map attendance records to date string (YYYY-MM-DD)
              const attendanceMap = {};
              if (Array.isArray(applicant.attendance)) {
                applicant.attendance.forEach(record => {
                  // Always convert record.date to YYYY-MM-DD for mapping
                  let recordDate = record.date;
                  if (recordDate instanceof Date) {
                    recordDate = recordDate.toISOString().split("T")[0];
                  }
                  attendanceMap[recordDate] = record.remark;
                });
              }
              return {
                user_id: applicant.user_id,
                full_name: applicant.full_name,
                attendance: tableDates.map(dateStr => ({
                  date: dateStr,
                  remark: attendanceMap[dateStr] || "absent"
                }))
              };
            });
            setAllUsersAttendance(allAttendance);
          } catch (error) {
            // ...existing code...
          }
        });
      ...
      <div className="program-details-content">
        <div className="program-header">
          ...
          <div className="lg:hidden relative">
            ...
            {/* Attendance Table - Only in Attendance Tab */}
            {/* ...existing code... */}
          {/* Tab Content */}
                  ...
                  <table className="min-w-full border-collapse text-xs sm:text-sm text-center">
                    <thead className="bg-gray-100">
                      <tr>
                        ...
                        {tableDates.map((date, idx) => (
                          <th key={idx} className="px-2 sm:px-4 py-2">
                    ...
                    <tbody>
                      {approvedApplicants.map(applicant => (
                        <tr key={applicant.id}>
                          <td className="px-2 sm:px-4 py-2">{applicant.name || applicant.full_name}</td>
                          {tableDates.map((date, idx) => {
                            const dateStr = date.toISOString().split("T")[0];
                            const attendanceRecord = attendanceData[applicant.id]?.find(a => {
                              let attDate;
                              if (a.date?.seconds) attDate = new Date(a.date.seconds * 1000).toISOString().split("T")[0];
                              else if (a.date?._seconds) attDate = new Date(a.date._seconds * 1000).toISOString().split("T")[0];
                              else if (typeof a.date === "number") attDate = new Date(a.date * 1000).toISOString().split("T")[0];
                              else if (a.date instanceof Date) attDate = a.date.toISOString().split("T")[0];
                              else attDate = a.date;
                              return attDate === dateStr;
                            });
                            return (
                              <td key={idx} className={`px-2 sm:px-4 py-2 capitalize text-center ${attendanceRecord?.status === "present" ? "text-green-600" : "text-red-600"}`}>
            ...
            {activeTab === "attendance" && (
              <div className="table-container bg-white shadow-lg rounded-lg p-4 sm:p-6">
                <div className="attendance-tab">
                  {(userRole === "admin" || userRole === "trainer" || (requestorType?.toLowerCase() === "facilitator" && userId === requestorId)) ? (
                    <div>
                      <h2 className="text-base sm:text-lg font-semibold mb-2">Attendance Overview</h2>
                          ...
                          <thead className="bg-gray-100">
                            <tr>
                              ...
                              {(programDetails.selected_dates?.length > 0 ? programDetails.selected_dates : dateRange).map(
                                (date) => {
                                  const formattedDate = (date.seconds ? new Date(date.seconds * 1000) : new Date(date)).toISOString().split("T")[0];
                                  return (
